...
* **`reference/psxsym-candidates.tsv`** — 7 names the TU alignment can still suggest
...
* **`reference/psxsym-unplaced.tsv`** — 55 functions PSX.SYM knows that we could not
  ...
  `DrawSpriteXYZ`, `GetScreenPositionS`, `PutMoji*`, `QuakeCamera` — are real game code and
  should be findable; they are the best remaining targets.

* **`reference/psxsym-unnamed.tsv`** — 84 retail placeholders with no candidate at
